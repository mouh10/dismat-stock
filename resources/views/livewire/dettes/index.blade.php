<div class="space-y-4">
    <x-page-header title="Dettes fournisseurs"
        subtitle="{{ $nbTotal }} dette(s) en cours - <span class='font-semibold text-red-600'>{{ number_format($totalRestant, 0, ',', ' ') }} F CFA</span> restant" />
    ...
    <div class="card overflow-x-auto">
        <table class="w-full text-sm table-modern">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($dettes as $d)
                    @php $enRetard = $d->date_echeance && $d->date_echeance->isPast(); @endphp
                    <tr wire:key="dette-{{ $d->id }}" class="{{ $enRetard ? 'bg-red-50/40' : '' }}">
                        <td class="px-4 py-3 font-medium">{{ $d->fournisseur?->nom }}</td>
                        <td class="px-4 py-3">{{ number_format($d->montant_initial, 0, ',', ' ') }} F</td>
                        <td class="px-4 py-3 text-red-600 font-medium">{{ number_format($d->montant_restant, 0, ',', ' ') }} F</td>
                        <td class="px-4 py-3 {{ $enRetard ? 'text-red-600 font-medium' : 'text-slate-500' }}">{{ $d->date_echeance?->format('d/m/Y') ?? '—' }}</td>
                        <td class="px-4 py-3">
                            @if ($enRetard)
                                <span class="badge bg-red-600 text-white">En retard</span>
                            @else
                                <span class="badge bg-red-50 text-red-700 capitalize">{{ $d->statut }}</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-right">
                            <button wire:click="openPay({{ $d->id }})" class="text-brand-700 hover:underline font-medium">Payer</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-slate-400">Aucune dette ne correspond à ces filtres.</td></tr>
                @endforelse
            </tbody>
        </table>
        @if ($dettes->hasPages())
            <div class="p-3">{{ $dettes->links() }}</div>
        @endif
    </div>
    ...
    @if ($showPayModal)
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 backdrop-enter" wire:click.self="$set('showPayModal', false)">
            <div class="bg-white rounded-2xl shadow-2xl modal-enter w-full max-w-sm p-5">
                <h3 class="font-display font-semibold text-lg text-ink-950 mb-4">Payer un fournisseur</h3>
