# G-Stock — Laravel Edition

Réécriture de l'application **G-Stock** (gestion de stock, caisse/POS, achats,
clients, fournisseurs, créances/dettes, trésorerie) en **Laravel 12 + Livewire 3
+ Blade + Tailwind CSS + PostgreSQL**, à partir de l'application d'origine (Next.js
+ Supabase).

## ⚠️ Important avant de démarrer

Ce projet a été écrit intégralement "à la main" (migrations, modèles, composants
Livewire, vues Blade) car l'environnement qui l'a généré n'avait pas accès à
`packagist.org` pour exécuter `composer install`. **Le code n'a donc pas encore
été exécuté ni testé en conditions réelles.** Suis les étapes ci-dessous pour
l'installer chez toi ; il est possible qu'une erreur mineure apparaisse au
premier lancement (nom de colonne, import manquant...) — dis-le-moi et je la
corrigerai.

## Stack technique

- **Laravel 12** (PHP 8.2+)
- **Livewire 3** pour l'interactivité (pas de JS custom, tout en PHP/Blade)
- **PostgreSQL** comme base de données
- **Tailwind CSS v4** (déjà configuré via Vite, aucune config supplémentaire requise)
...
# 2. Dépendances front (Tailwind/Vite)
npm install
...
# 4. Configurer PostgreSQL dans .env (déjà pré-rempli avec des valeurs par défaut)
#    DB_DATABASE=gstock / DB_USERNAME=postgres / DB_PASSWORD=postgres
#    -> Crée la base au préalable : createdb gstock

# 5. Lancer les migrations
php artisan migrate

# 6. Créer TA boutique et ton compte admin
#    -> Ouvre d'abord database/seeders/DatabaseSeeder.php et remplace
#       nom / email / mot de passe par tes vraies informations, PUIS :
php artisan db:seed

# 7. Build des assets puis lancement
npm run build
php artisan serve
```

Ouvre ensuite `http://localhost:8000` et connecte-toi avec l'email/mot de passe
que tu as mis dans `DatabaseSeeder.php`. Il n'y a pas de page d'inscription :
c'est **ta** boutique, un seul compte est créé au démarrage (tu pourras en
ajouter d'autres ensuite depuis le module **Équipe**).

Pour le développement au quotidien, utilise plutôt :
```bash
composer run dev
# lance en parallèle : serveur PHP, queue, logs et Vite (hot reload)
```
